...
uint32_t SiriusClientCore::gCnt = 0;

SiriusClientCore *SiriusClientCore::gThis = NULL;

pthread_mutex_t SiriusClientCore::mInsL   = PTHREAD_MUTEX_INITIALIZER;

SiriusClientCore *SiriusClientCore::getInstance()
{
    pthread_mutex_lock(&mInsL);

    if (ISNULL(gThis)) {
        gThis = new SiriusClientCore();
        if (NOTNULL(gThis)) {
            if (!SUCCEED(gThis->construct())) {
                delete gThis;
                gThis = NULL;
            }
        }
...
uint32_t SiriusClientCore::removeInstance()
{
    ASSERT_LOG(MODULE_SIRIUS_CLIENT_CORE,
        gCnt > 0, "Instance not got.");
    ASSERT_LOG(MODULE_SIRIUS_CLIENT_CORE,
        NOTNULL(gThis), "Instance not created.");
...
int32_t SiriusClientCore::findMemoryMap(
...
int32_t SiriusClientCore::releaseAllMems()
{
    int32_t rc = NO_ERROR;
    int32_t num = REQUEST_TYPE_MAX_INVALID *
        REQUEST_HANDLER_MAX_MEMORY_NUM;
...
int32_t SiriusClientCore::onDataReady(
...
int32_t SiriusClientCore::copyToServer(ClientRequestIntf *intf,
...
int32_t SiriusClientCore::runOnceFunc(void * /*in*/, void * /*out*/)
{
    int32_t rc = NO_ERROR;
    int32_t fd = -1;
    int32_t size = 0;
...
int32_t SiriusClientCore::acceptSingleRequestMemory()
{
    int32_t rc = NO_ERROR;
...
int32_t SiriusClientCore::addMemoryMap(RequestType type,
...
int32_t SiriusClientCore::waitForPushRequestMemory()
{
    int32_t rc = NO_ERROR;
...
int32_t SiriusClientCore::onOnceFuncFinished(int32_t /*rc*/)
{
    return NO_ERROR;
}

int32_t SiriusClientCore::abortOnceFunc()
{
    mAbortOnce = true;
    mSC.cancelWaitMsg();
    return NO_ERROR;
...
int32_t SiriusClientCore::RunOnce::run(RunOnceFunc *func, void *in, void *out)
{
    int32_t rc = NO_ERROR;
...
int32_t SiriusClientCore::RunOnce::exit()
{
    return RunOnceThread::exit();
}

bool SiriusClientCore::RunOnce::isRuning()
{
    return RunOnceThread::isRuning();
}

int32_t SiriusClientCore::ReadyNotifier::init()
{
    int32_t rc = NO_ERROR;
...
int32_t SiriusClientCore::ReadyNotifier::deinit()
{
    mExit = true;
    return destruct();
}

int32_t SiriusClientCore::ReadyNotifier::notify(
...
SiriusClientCore::ReadyNotifier::ReadyNotifier() :
    mExit(false)
{
}
